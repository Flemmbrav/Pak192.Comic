...
#Parameters
	...
	if [[ $dat =~ "Stadtbahn_Frankfurt" || $dat =~ "Stadtbahn_Rhein-Ruhr" || $dat =~ "Stuttgarter_Stadtbahn" ]] ; then
		ObjectArray[waytype]="tram_track"
	elif [[ ${ObjectArray[waytype]} == "track" ]] ;then
		...
		if [[ ${ObjectArray[obj]} == "vehicle" || ${ObjectArray[obj]} == "Vehicle" ]];	then
			if [[ ${ObjectArray[freight]} == "Passagiere" || ${ObjectArray[freight]} == "passagiere"  || ${ObjectArray[freight]} == "Post"  || ${ObjectArray[freight]} == "post"  || ${ObjectArray[freight]} == "None"  || ${ObjectArray[freight]} == "none" || -z ${ObjectArray[freight]} ]];	then
				...
				if [[ $AddOnFolder == 1 ]] ;then
					local calculateddir=../Pak128Britain/AddOn/$(dirname "$dat")/
				else
					local calculateddir=../Pak128Britain/$(dirname "$dat")/
				fi
				...
				if [[ $AddOnFolder == "1" ]] ;then
					calculateddir="../Pak128Britain/AddOn"
					#echo "true"
				else
					calculateddir="../Pak128Britain"
					#echo "false"
				fi
